...
| POST | `/v1/instrument-valuation-analysis/initiate` | Initiate |
| GET | `/v1/instrument-valuation-analysis` | Retrieve (list) |
| GET | `/v1/instrument-valuation-analysis/{crId}/retrieve` | Retrieve |
| PUT | `/v1/instrument-valuation-analysis/{crId}/update` | Update |
| PUT | `/v1/instrument-valuation-analysis/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/instrument-valuation-analysis/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-financial-instrument-valuation:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-financial-instrument-valuation ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-financial-instrument-valuation` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
